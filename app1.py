...
def what_if_analysis(df, model, scaler, selected_features):
    st.title("🔮 What If Analysis")
    st.write("Explore how changes in different factors might affect life expectancy predictions.")

    # Select a baseline county
    st.sidebar.subheader("Select Baseline County")
    selected_state = st.sidebar.selectbox("State", sorted(df['state'].unique()))
    counties = df[df['state'] == selected_state]['geo_name'].unique()
    selected_county = st.sidebar.selectbox("County", sorted(counties))

    # Get the baseline data for the selected county
    baseline_data = df[(df['state'] == selected_state) & (df['geo_name'] == selected_county)].iloc[0]
    ...
    for feature in selected_features:
        ...
        adjusted_values[feature] = st.sidebar.slider(
            feature.replace('_', ' ').title(),
            min_value=float(min_val),
            max_value=float(max_val),
            value=float(baseline_data[feature]),
            step=step,
            key=f"what_if_{feature}"
        )

    # Prepare data for prediction
    baseline_input = pd.DataFrame([baseline_data[selected_features]], columns=selected_features)
    adjusted_input = pd.DataFrame([adjusted_values], columns=selected_features)

    # Scale inputs
    baseline_scaled = scaler.transform(baseline_input)
    adjusted_scaled = scaler.transform(adjusted_input)

    # Make predictions
    baseline_prediction = model.predict(baseline_scaled)[0]
    adjusted_prediction = model.predict(adjusted_scaled)[0]
    ...
    with col1:
        ...
        st.markdown(f"<h1 style='text-align: center; color: #4e79a7;'>{baseline_prediction:.2f} years</h1>", unsafe_allow_html=True)
        st.write(f"Based on current factors in {selected_county}, {selected_state}")

    with col2:
        st.subheader("Adjusted Prediction")
        st.markdown(f"<h1 style='text-align: center; color: #4e79a7;'>{adjusted_prediction:.2f} years</h1>", unsafe_allow_html=True)
        st.write("Based on your adjustments")

    # Calculate and display the change
    change = adjusted_prediction - baseline_prediction
    change_color = "green" if change > 0 else "red"
    st.markdown(f"<h2 style='text-align: center; color: {change_color};'>Change: {change:.2f} years</h2>", unsafe_allow_html=True)

    # Create a radar chart to visualize the changes
    categories = [feature.replace('_', ' ').title() for feature in selected_features]
    baseline_values = [baseline_data[feature] for feature in selected_features]
    adjusted_values = [adjusted_values[feature] for feature in selected_features]
    ...
    fig.update_layout(
        ...
        showlegend=True,
        title="Comparison of Baseline and Adjusted Factors"
    )
    ...
    st.write("""
    This analysis shows how changes in various factors might affect life expectancy. 
    The radar chart visualizes the differences between the baseline and adjusted values for each factor. 
    Areas where the adjusted values extend beyond the baseline indicate potential improvements, 
    while areas where they fall short suggest possible negative impacts.
    
    Remember that this is a simplified model and real-world outcomes may be influenced by many other factors. 
    Always consult with health professionals and policymakers for comprehensive health strategies.
    """)
    
# Main function to run the app
def main():
    st.sidebar.title("Navigation")
    page = st.sidebar.radio("Select a page", ["Overview", "Life Expectancy Insights", "Disparities and Impact", "Life Expectancy Predictor", "What If Analysis"])
    ...
    if page == "Overview":
        ...
        predict_life_expectancy()
            ...
            what_if_analysis(df, model, scaler, selected_features)
